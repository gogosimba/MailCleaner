...
start_phrase = '<!-- wp:paragraph {"blockVisibility":{"controlSets":[{"id":1,"enable":true,"controls":{"userRole":{"visibilityByRole":"user-role","restrictedRoles":["administrator","editor"]}}}]}} -->'
end_phrase = "<!-- /wp:paragraph -->"
...
def clean_volvouppdrag_email(email_body):
    ...
    # We will keep this one in mind, but for now relevant code below =)
...
def clean_combitech_email(email_body):
    ...
    # Remove any ID that is between 4 and 6 digits long
...
{"userRole":{"visibilityByRole":"user-role","restrictedRoles":["administrator","editor"]}}}]}} -->
<p>kontakta olaconny för mer detaljer</p>
<!-- /wp:paragraph -->"""
